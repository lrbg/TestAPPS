...
class Seats {
    /** UI Selectors */
    get seatContainer() {
        return $('android=new UiSelector().className("android.widget.ScrollView").index(3)');
    }

    get continueButton() {
        return $('android=new UiSelector().descriptionContains("Continuar con")');
    }

    get seatElements() {
        return $$('android=new UiSelector().className("android.view.ViewGroup").clickable(true).enabled(true).focusable(true)');
    }

    get objPink() {
        return $('android=new UiSelector().className("android.widget.TextView").textContains("Confirmo que el asiento seleccionado será utilizado por una mujer")');
    }

    get chkPink() {
        return $('android=new UiSelector().className("android.widget.ImageView").bounds("[79,1627][142,1690]")');
    }

    get btnNoSetSeats() {
        return $('android=new UiSelector().className("android.view.ViewGroup").description("Elige al menos 1 asiento")');
    }
    ...
    async processSeats(numberOfSeatsToSelect) {
        await this.seatContainer.waitForDisplayed({ timeout: 10000 });
